...
public class SwaggerConfig extends WebMvcConfigurationSupport {
	...
	public Docket productApi(ServletContext servletContext) {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.ksewa.springsecurity")).paths(regex("/.*")).build()
				.apiInfo(metaData()).pathProvider(new RelativePathProvider(servletContext) {
					@Override
					public String getApplicationBasePath() {
						return "ksewa-service";
					}
				});
	}
	...
	private ApiInfo metaData() {
		return new ApiInfo("KSewa RestCall Docs", "Ksewa-Service", "1.0", "Terms of service",
				"Apache License Version 2.0", "https://www.apache.org/licenses/LICENSE-2.0", "K-Sewa");
	}
